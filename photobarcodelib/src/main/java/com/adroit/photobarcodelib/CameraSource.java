...
public class CameraSource {
    ...
    static int DEFAULT_PREVIEW_WIDTH = 1024;
    static int DEFAULT_PREVIEW_HEIGHT = 768;
...
//            }
        ...
        //set image size closest to this
        public Builder setMinImageSize(int size) {
            mCameraSource.mMinImageSize = size;
            return this;
        }
     ...
     */
    public CameraSource start(SurfaceHolder surfaceHolder) throws IOException, RuntimeException {
        synchronized (mCameraLock) {
            if (mCamera != null) {
                return this;
            }
    ...
    public boolean setFocusMode(@FocusMode String mode) {
        synchronized (mCameraLock) {
            if (mCamera != null && mode != null) {
                Camera.Parameters parameters = mCamera.getParameters();
                if (parameters.getSupportedFocusModes() != null && parameters.getSupportedFocusModes().contains(mode)) {
     ...
     * @return {@code true} if the focus areas is set, {@code false} otherwise
     */
    public boolean setFocusAreas(List<Camera.Area> focusAreas) {
        synchronized (mCameraLock) {
            if (mCamera != null && focusAreas != null && !focusAreas.isEmpty()) {
                Camera.Parameters parameters = mCamera.getParameters();
    ...
    public boolean setShutterSound(boolean enable) {
    ...
    private class PictureDoneCallback implements Camera.PictureCallback {
        ...
        public void onPictureTaken(byte[] data, Camera camera) {
            ...
            //if you need multiple photos, uncomment
            //restartPreview();
        }
    }
    ...
    public void resetCameraRotation(){
        synchronized (mCameraLock) {
            if (mCamera != null) {
                Camera.Parameters parameters = mCamera.getParameters();
                setRotation(mCamera, mCamera.getParameters(), getIdForRequestedCamera(mFacing));
                mCamera.setParameters(parameters);
            }
        }
    }
    ...
    private Camera createCamera() throws RuntimeException{
        ...
        try {
            ...
            //to avoid ex
            //java.lang.RuntimeException: Camera initialization failed because the camera device was already opened(-16)
            if (mCamera != null) {
                camera = mCamera;
            } else {
                throw ex;
        ...
        SizePair sizePair = selectSizePair(camera, mRequestedPreviewWidth, mRequestedPreviewHeight, mMinImageSize);
        if (sizePair == null) {
        ...
        if (mFrameProcessor != null && mFrameProcessor.mDetector != null) {
     ...
     */
    private static SizePair selectSizePair(Camera camera, int desiredWidth, int desiredHeight, int minImageSize) {
        ...
        double targetRatio = (double) Math.max(desiredWidth, desiredHeight) / Math.min(desiredWidth, desiredHeight);
        if (validPreviewSizes == null) return null;


        List<SizePair> filterAspect = filter(validPreviewSizes, x -> Math.abs(((double) x.mPreview.getWidth() / x.mPreview.getHeight()) - targetRatio) <= ASPECT_TOLERANCE);
        List<SizePair> filterAspectSize = filter(filterAspect, x -> Math.max(x.mPicture.getWidth(), x.mPicture.getHeight()) >= minImageSize);

        SizePair optimalSize = min(filterAspectSize, y -> Math.abs(y.mPreview.getHeight() - desiredHeight));
        if(optimalSize == null){
            optimalSize = min(filterAspect, y -> Math.abs(y.mPreview.getHeight() - desiredHeight));
        }
        if(optimalSize == null){
            optimalSize = min(validPreviewSizes, x->Math.abs(x.mPreview.getWidth() - desiredWidth) + Math.abs(x.mPreview.getHeight() - desiredHeight));
        }
        return optimalSize;
    }

    public static <T> List<T> filter(List<T> list, Function<T, Boolean> compare) {
    ...
    public static <T> T max(List<T> list, Function<T, Integer> function) {
        return minMax(list, function, false);
    }
    public static <T> T min(List<T> list, Function<T, Integer> function) {
    ...
    **/
    ...
    public static int getDegreesBySurfaceRotation(int surfaceRotation){
        int degrees = 0;
        switch (surfaceRotation) {
            ...
            case Surface.ROTATION_270:
                ...
                break;
            default:
                Log.e(TAG, "Bad rotation value: " + surfaceRotation);
        }
        return degrees;
    }
    ...
    private void setRotation(Camera camera, Camera.Parameters parameters, int cameraId) {
        ...
        int degrees = getDegreesBySurfaceRotation(rotation);
